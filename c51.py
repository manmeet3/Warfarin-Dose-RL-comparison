...
class C51DQN:
    ...
    def build_net(self, state, action, c_names, units_1, units_2, w_i, b_i, reg=None):
        with tf.variable_scope('conv1'):
            conv1 = conv(state, [5, 3, 3, 6], [6], [1, 2, 2, 1], w_i, b_i)
    ...
    def train(self, s, r, action, s_, gamma):
        ...
        for j in range(self.atoms):
            ...
            m[int(l)] += pj * (u - bj)
            m[int(u)] += pj * (bj - l)
        self.sess.run(self.optimize, feed_dict={self.state_input: [s], self.action_input: [action],
            self.m_input: m})

    @lazy_property
    def optimize(self):
        optimizer = tf.train.AdamOptimizer(self.config.LEARNING_RATE)
        return optimizer.minimize(self.cross_entropy_loss)
    ...
    def greedy_action(self, s):
        ...
        return np.argmax([self.Q.eval(feed_dict={self.state_input: [s], self.action_input: [[a]]})
